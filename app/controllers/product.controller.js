...
exports.getall = async (req, res) => {
  try {
    productService.getall((err, dataRes) => {
          if (err)
...
exports.getById = async (req, res) => {
  try {
      ...
      productService.getById(id, (err, dataRes) => {
          if (err) {
              return res.send({ result: false, error: [err] });
          }
...
exports.create = async (req, res) => {
  try {
      ...
      productService.create(data, (errss, dataRes) => {
          if (errss)
              return res.send({
                  result: false,
                  error: [errss]
...
exports.update = async (req, res) => {
    try {
        ...
        productService.getById(id, async (err, dataRes) => {
            if (err) {
                return res.send({ result: false, error: [err] });
            }
            ...
            productService.update(id, data, (errss, dataRes) => {
                if (errss)
                    return res.send({
                        result: false,
                        error: [errss]
                ...
                // Sau khi cập nhật thành công, lấy dữ liệu mới bằng cách gọi hàm getProductById hoặc getall
                productService.getall((err, newDataRes) => {
                    if (err)
                        return res.send({ result: false, error: [err] });
...
exports.delete = async (req, res) => {
    try {
      ...
      productService.getall((err, initialData) => {
        if (err)
          return res.send({ result: false, error: [err] });
  
        productService.delete(id, (err, deletedData) => {
          if (err) {
            return res.send({ result: false, error: [err] });
          }
          ...
          productService.getall((err, newData) => {
            if (err)
              return res.send({ result: false, error: [err] });
            return res.send({
              result: true,
